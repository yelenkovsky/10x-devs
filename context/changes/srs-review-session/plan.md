...
- [ ] 1.1 New migration from `npx supabase migration new add_flashcard_fsrs` adds the nullable FSRS columns and queue index; it applies locally
- [ ] 1.2 `ts-fsrs` is a runtime dependency in `package.json`
- [ ] 1.3 `src/pages/api/review.ts` exports `const prerender = false`, `GET`, and `POST`
- [ ] 1.4 `src/types.ts` exports the review session request and response types
- [ ] 1.5 `FLASHCARD_COLUMNS` and the dashboard `Flashcard` type stay content-only (no FSRS fields)
- [ ] 1.6 Un-keep UPDATE nulls FSRS columns in the same write as `status: "generated"`
- [ ] 1.7 `npx astro sync` succeeds
- [ ] 1.8 `npm run lint` succeeds
- [ ] 1.9 `npm run build` succeeds
