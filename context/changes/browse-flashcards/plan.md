...
- [ ] 1.1 Inbox `listFlashcards` orders `created_at` desc then `id` desc and still returns `Flashcard[]` with no cap
- [ ] 1.2 Browse helper filters status and `word_phrase` before applying the 1000 cap and returns `{ cards, capped }`
- [ ] 1.3 `src/types.ts` exports the browse result and status-filter types
...
- [ ] 1.7 `/dashboard` still lists every owned status, newest first; same-timestamp twins stay in the same order across refresh
- [ ] 1.8 Browse helper with `status: "generated"` returns only generated rows for that user
- [ ] 1.9 Non-empty `q` matches `word_phrase` case-insensitively and does not match on definition/cloze alone
- [ ] 1.10 A 1001st matching row is omitted and `capped` is true; an exact 1000 matching rows leaves `capped` false
- [ ] 1.11 User B’s client never returns user A’s rows

### Phase 2: `/cards` catalog

#### Automated

- [ ] 2.1 `src/pages/cards.astro` exists and hydrates a `client:load` browse island
- [ ] 2.2 `PROTECTED_ROUTES` includes `/cards`
- [ ] 2.3 Filter chrome (status chips + word/phrase find) lives on `/cards`, not `/dashboard`
- [ ] 2.4 Paste-generate and hand-create remain on `/dashboard` only
...
- [ ] 3.1 Signed-in `Topbar` links to `/dashboard`, `/cards`, and `/review`
- [ ] 3.2 `/dashboard`, `/review`, and `/cards` mount `Topbar`; auth pages do not
- [ ] 3.3 Product page headers no longer duplicate those nav links or Sign out
