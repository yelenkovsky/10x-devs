...
- [ ] 1.1 `src/lib/services/paste-targets.test.ts` covers list vs prose, cap, truncated, and grounding cases above
- [ ] 1.2 `generate-cards` tests persist only grounded cards and set `truncated` from input cut, including the 3-item + 15-envelope case
- [ ] 1.3 `npm test` exits 0
- [ ] 1.4 `npm run lint` succeeds
- [ ] 1.5 `npx astro sync` succeeds
- [ ] 1.6 `npm run build` succeeds

#### Manual

- [ ] 1.7 Local signed-in generate of a 3-word slash list (live key) yields cards whose `wordPhrase`s appear in the paste — not 15 unrelated targets
